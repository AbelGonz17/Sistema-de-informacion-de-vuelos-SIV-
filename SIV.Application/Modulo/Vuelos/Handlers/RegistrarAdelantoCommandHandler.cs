    ...
    public class RegistrarAdelantoCommandHandler : IRequestHandler<RegistrarAdelantoCommand, Result<bool>>
    {
        ...
        public async Task<Result<bool>> Handle(RegistrarAdelantoCommand request, CancellationToken cancellationToken)
        {
            ...
            if (request.NuevaHoraSalida >= horaBase)
                return Result<bool>.Failure($"La nueva hora ({request.NuevaHoraSalida:dd/MM/yyyy HH:mm}) debe ser estrictamente anterior a la hora programada/estimada actual ({horaBase:dd/MM/yyyy HH:mm}) para registrarse como adelanto.", StatusCodes.Status400BadRequest);
